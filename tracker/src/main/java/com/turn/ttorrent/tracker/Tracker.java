 ...
 * <p>
 *
 * <p>The tracker usually listens on port 6969 (the standard BitTorrent tracker port). Torrents must
 * be registered directly to this tracker with the {@link #announce(TrackedTorrent torrent)}</code>
 * method.
 *
 * @author mpetazzoni
 */
public class Tracker {
    ...
    public Tracker(int port) throws IOException {
        this(
                port,
                getDefaultAnnounceUrl(new InetSocketAddress(InetAddress.getLocalHost(), port))
                        .toString());
    }
    ...
    public Tracker(int port, String announceURL) throws IOException {
        ...
        final TrackerRequestProcessor requestProcessor =
                new TrackerRequestProcessor(myTorrentsRepository);
        myTrackerServiceContainer =
                new TrackerServiceContainer(
                        requestProcessor, new MultiAnnounceRequestProcessor(requestProcessor));
    ...
    public Tracker(
            int port,
            String announceURL,
            TrackerRequestProcessor requestProcessor,
     ...
     * <p>
     *
     * <p>This has the form http://host:port/announce.
     */
    ...
    public void start(final boolean startPeerCleaningThread) throws IOException {
        ...
        List<SocketAddress> tries =
                new ArrayList<SocketAddress>() {
                    {
                        try {
                            add(
                                    new InetSocketAddress(
                                            InetAddress.getByAddress(new byte[4]), myPort));
                        } catch (Exception ex) {
                        }
                        try {
                            ...
                            add(
                                    new InetSocketAddress(
                                            InetAddress.getByName(
                                                    new URL(getAnnounceUrl()).getHost()),
                                            myPort));
                        } catch (Exception ex) {
                        }
                    }
        ...
        for (SocketAddress address : tries) {
            try {
                ...
                logger.info(
                        "Can't start the tracker using address{} : ",
                        address.toString(),
                        ioe.getMessage());
            }
        }
        ...
        if (startPeerCleaningThread) {
            if (myPeerCollectorThread == null
                    || !myPeerCollectorThread.isAlive()
                    || myPeerCollectorThread.getState() != Thread.State.NEW) {
                myPeerCollectorThread = new PeerCollectorThread(myTorrentsRepository);
            }
     ...
     * <p>
     *
     * <p>This effectively closes the listening HTTP connection to terminate the service, and
     * interrupts the peer myPeerCollectorThread thread as well.
     */
    public void stop() {
        this.stop = true;
     ...
     * <p>
     *
     * <p>The fact that torrents must be announced here first makes this tracker a closed BitTorrent
     * tracker: it will only accept clients for torrents it knows about, and this list of torrents
     * is managed by the program instrumenting this Tracker class.
     *
    ...
    public synchronized TrackedTorrent announce(TrackedTorrent torrent) {
        ...
        if (existing != null) {
            logger.warn(
                    "Tracker already announced torrent with hash {}.", existing.getHexInfoHash());
            return existing;
        }
    ...
    /**
     * Set to true to allow this tracker to track external torrents (i.e. those that were not
     * explicitly announced here).
     *
