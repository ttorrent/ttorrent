 ...
 * <p>
 *
 * <p>It only serves announce requests on /announce, and only serves torrents the {@link Tracker} it
 * serves knows about.
 *
 * <p>
 *
 * <p>The list of torrents {@see #requestHandler.getTorrentsMap()} is a map of torrent hashes to
 * their corresponding Torrent objects, and is maintained by the {@link Tracker} this service is
 * part of. The TrackerRequestProcessor only has a reference to this map, and does not modify it.
 *
 * @author mpetazzoni
...
public class TrackerRequestProcessor {
    ...
    public TrackerRequestProcessor(
            TorrentsRepository torrentsRepository, AddressChecker addressChecker) {
     ...
     * <p>
     *
     * <p>This method attemps to read and parse the incoming announce request into an announce
     * request message, then creates the appropriate announce response message and sends it back to
     * the client.
     */
    public void process(final String uri, final String hostAddress, RequestHandler requestHandler)
         ...
         *
         * <p>We need to rely on our own query parsing function because SimpleHTTP's Query map will
         * contain UTF-8 decoded parameters, which doesn't work well for the byte-encoded strings we
         * expect.
         */
        ...
        try {
            ...
            LoggerUtils.warnAndDebugDetails(
                    logger, "Unable to parse request message. Request url is {}", uri, mve);
        ...
        // The requested torrent must be announced by the tracker if and only if
        // myAcceptForeignTorrents is false
        if (!myAcceptForeignTorrents && torrent == null) {
            logger.warn("Requested torrent hash was: {}", announceRequest.getHexInfoHash());
            serveError(
                    Status.BAD_REQUEST, ErrorMessage.FailureReason.UNKNOWN_TORRENT, requestHandler);
            return;
        }
        ...
        try {
            torrent =
                    myTorrentsRepository.putIfAbsentAndUpdate(
            ...
            LoggerUtils.warnAndDebugDetails(
                    logger, "Unable to update peer torrent. Request url is {}", uri, iae);
            serveError(
                    Status.BAD_REQUEST, ErrorMessage.FailureReason.INVALID_EVENT, requestHandler);
            return;
        }
    ...
    private void writeEmptyResponse(
            HTTPAnnounceRequestMessage announceRequest, RequestHandler requestHandler)
            throws IOException {
        HTTPAnnounceResponseMessage announceResponse;
        try {
            announceResponse =
                    HTTPAnnounceResponseMessage.craft(
                            myAnnounceInterval,
                            0,
                            0,
                            Collections.<Peer>emptyList(),
                            announceRequest.getHexInfoHash());
            requestHandler.serveResponse(
                    Status.OK.getCode(), Status.OK.getDescription(), announceResponse.getData());
        } catch (Exception e) {
    ...
    private void writeAnnounceResponse(
            TrackedTorrent torrent, Peer peer, boolean isSeeder, RequestHandler requestHandler)
            throws IOException {
        HTTPAnnounceResponseMessage announceResponse;
        try {
            announceResponse =
     ...
     * <p>
     *
     * <p>Because we're expecting byte-encoded strings as query parameters, we can't rely on
     * SimpleHTTP's QueryParser which uses the wrong encoding for the job and returns us unparsable
     * byte data. We thus have to implement our own little parsing method that uses BYTE_ENCODING to
     * decode parameters from the URI.
     *
     * <p>
     *
     * <p><b>Note:</b> array parameters are not supported. If a key is present multiple times in the
     * URI, the latest value prevails. We don't really need to implement this functionality as this
     * never happens in the Tracker HTTP protocol.
     *
     * @param uri
     * @param hostAddress
    ...
    private HTTPAnnounceRequestMessage parseQuery(final String uri, final String hostAddress)
        ...
        if (params.get("ip") == null) {
            params.put("ip", new BEValue(hostAddress, Constants.BYTE_ENCODING));
        }
    ...
    private void serveError(
            Status status, HTTPTrackerErrorMessage error, RequestHandler requestHandler)
            throws IOException {
     ...
     */
    private void serveError(Status status, String error, RequestHandler requestHandler)
            throws IOException {
     ...
     */
    private void serveError(
            Status status, ErrorMessage.FailureReason reason, RequestHandler requestHandler)
            throws IOException {
