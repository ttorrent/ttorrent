...
public class TrackerTest {
    ...
    private List<CommunicationManager> communicationManagerList =
            new ArrayList<CommunicationManager>();
    ...
    public void testPeerWithManyInterfaces() throws Exception {
        ...
        final InetAddress[] inetAddresses =
                selfAddresses.toArray(new InetAddress[selfAddresses.size()]);
                ...
                new WaitFor(10000) {
                    ...
                    protected boolean condition() {
                        final TrackedTorrent trackedTorrent =
                                tracker.getTrackedTorrent(hexInfoHash);
                        return trackedTorrent != null
                                && trackedTorrent.getPeers().size() >= inetAddresses.length;
                    }
                };
    ...
    public void test_share_and_download() throws IOException, InterruptedException {
        ...
        assertEquals(
                tt.getHexInfoHash(),
                seeder.getTorrentsStorage()
                        .announceableTorrents()
                        .iterator()
                        .next()
                        .getHexInfoHash());
        ...
        try {
            ...
            assertFilesEqual(
                    new File(TEST_RESOURCES + "/parentFiles/file1.jar"),
                    new File(downloadDir, "file1.jar"));
        } finally {
            leech.stop();
            seeder.stop();
        }
    ...
    public void tracker_removes_peer_after_peer_shutdown()
        ...
        final InetSocketAddress c1Address =
                new InetSocketAddress(
                        InetAddress.getLocalHost(), c1.getConnectionManager().getBindPort());
        final InetSocketAddress c2Address =
                new InetSocketAddress(
                        InetAddress.getLocalHost(), c2.getConnectionManager().getBindPort());
    ...
    public void tracker_removes_peer_after_timeout() throws IOException, InterruptedException {
        ...
        final String announceUrlC1 =
                "http://localhost:6969/announce?info_hash=%B9-8%04lv%D79H%E1LB%DF%99%2C%AF%25H%9D%08&peer_id=-TO0042-97ec308c9637&"
                    + "port="
                        + peerPort
                        + "&uploaded=0&downloaded=0&left=0&compact=1&no_peer_id=0&ip="
                        + peerHost;
    ...
    public void tracker_accepts_torrent_from_seeder_plus_leech()
            throws IOException, InterruptedException {
        this.tracker.setAcceptForeignTorrents(true);
    ...
    private TrackedTorrent loadTorrent(String name) throws IOException {
        TorrentMetadata torrentMetadata =
                new TorrentParser().parseFromFile(new File(TEST_RESOURCES + "/torrents", name));
    ...
    private SharedTorrent completeTorrent(String name) throws IOException {
        ...
        return SharedTorrent.fromFile(
                torrentFile,
                FairPieceStorageFactory.INSTANCE.createStorage(
                        torrentMetadata,
                        FileCollectionStorage.create(torrentMetadata, parentFiles)),
    ...
    private SharedTorrent incompleteTorrent(String name, File destDir) throws IOException {
        ...
        return SharedTorrent.fromFile(
                torrentFile,
                FairPieceStorageFactory.INSTANCE.createStorage(
                        torrentMetadata, FileCollectionStorage.create(torrentMetadata, destDir)),
                new TorrentStatistic());
    }
        ...
        new WaitFor() {
            ...
            protected boolean condition() {
                for (TrackedTorrent tt : tracker.getTrackedTorrents()) {
                    if (tt.seeders() == 1
                            && tt.getHexInfoHash()
                                    .equals(TorrentUtils.byteArrayToHexString(torrentHash)))
                        return true;
                }
