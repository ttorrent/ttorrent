...
public class TorrentTest {
    ...
    public void test_create_torrent() throws URISyntaxException, IOException, InterruptedException {
        ...
        TorrentMetadata t =
                TorrentCreator.create(
                        new File("src/test/resources/parentFiles/file1.jar"),
                        announceURI,
                        createdBy);
    ...
    public void load_torrent_created_by_utorrent() throws IOException {
        TorrentMetadata t =
                new TorrentParser()
                        .parseFromFile(new File("src/test/resources/torrents/file1.jar.torrent"));
    ...
    public void torrent_from_multiple_files()
        ...
        TorrentMetadata createdTorrent =
                TorrentCreator.create(
                        parentDir,
                        files,
                        announceURI,
                        ...
                        createdBy,
                        creationTimeSecs,
                        TorrentCreator.DEFAULT_PIECE_LENGTH);
        ...
        assertTrue(
                Hex.encodeHexString(expectedBytesWin).equals(Hex.encodeHexString(actualBytes))
                        || Hex.encodeHexString(expectedBytesLinux)
                                .equals(Hex.encodeHexString(actualBytes)));
    }
    ...
    public void testFilenames() throws IOException {
        ...
        for (TorrentFile torrentFileInfo : tmpFileNames) {
            normalizedFilenames.add(
                    t2.getDirectoryName()
                            + "/"
                            + torrentFileInfo.getRelativePathAsString().replaceAll("\\\\", "/"));
        }
        ...
        assertEqualsNoOrder(
                normalizedFilenames.toArray(new String[normalizedFilenames.size()]),
                expectedFilenames);
        System.out.println();
    }
}
