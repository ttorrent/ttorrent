...
public class CommunicationManagerFactory {
    ...
    public CommunicationManager getClient(String name) {
        ...
        final ExecutorService pieceValidatorExecutor =
                new ThreadPoolExecutor(
                        DEFAULT_POOL_SIZE,
                        DEFAULT_POOL_SIZE,
                        0L,
                ...
                if (timeout > 0) {
                    try {
                        if (!pieceValidatorExecutor.awaitTermination(timeout, timeUnit)) {
                            logger.warn(
                                    "unable to terminate executor service in {} {}",
                                    timeout,
                                    timeUnit);
                        }
                        boolean shutdownCorrectly =
                        ...
                        LoggerUtils.warnAndDebugDetails(
                                logger,
                                "unable to await termination executor service, thread was"
                                    + " interrupted",
                                e);
                    }
                }
