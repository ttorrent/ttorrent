    ...
    public void multiAnnounceTest() throws AnnounceException, ConnectException {
        List<Peer> peers =
                Collections.singletonList(
                        new Peer(new InetSocketAddress("127.0.0.1", 6881), ByteBuffer.allocate(1)));
        ...
        peers =
                Collections.singletonList(
                        new Peer(new InetSocketAddress("127.0.0.1", 6882), ByteBuffer.allocate(1)));
        client.multiAnnounce(AnnounceRequestMessage.RequestEvent.STARTED, true, torrents, peers);

        List<Peer> leecher =
                Collections.singletonList(
                        new Peer(new InetSocketAddress("127.0.0.1", 6885), ByteBuffer.allocate(1)));
        final AnnounceableInformation firstTorrentLeech = getMockedTorrent(new byte[] {1, 2, 3, 4});
        final AnnounceableInformation secondTorrentLeech =
                getMockedTorrent(new byte[] {1, 3, 3, 2});
        ...
        client.multiAnnounce(
                AnnounceRequestMessage.RequestEvent.STARTED,
                false,
                Arrays.asList(secondTorrentLeech, firstTorrentLeech),
                leecher);

        verify(listener, times(2))
                .handleAnnounceResponse(anyInt(), anyInt(), anyInt(), anyString());
        verify(listener, times(2))
                .handleDiscoveredPeers(ArgumentMatchers.<Peer>anyList(), anyString());
    }
