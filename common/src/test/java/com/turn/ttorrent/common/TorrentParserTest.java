...
public class TorrentParserTest {
    ...
    public void parseTest() throws IOException {
        ...
        final TorrentMetadata torrentMetadata =
                myTorrentParser.parse(BEncoder.bencode(metadata).array());
        ...
        announceList.add(
                new BEValue(Collections.singletonList(new BEValue("http://localhost/announce"))));
        announceList.add(
                new BEValue(Collections.singletonList(new BEValue("http://second/announce"))));
        metadata.put("announce-list", new BEValue(announceList));

        final TorrentMetadata torrentMetadataWithAnnounceList =
                myTorrentParser.parse(BEncoder.bencode(metadata).array());

        final List<List<String>> actualAnnounceList =
                torrentMetadataWithAnnounceList.getAnnounceList();
