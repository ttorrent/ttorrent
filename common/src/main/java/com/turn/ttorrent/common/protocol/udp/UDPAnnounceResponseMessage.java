    ...
    public static UDPAnnounceResponseMessage parse(ByteBuffer data)
        ...
        for (int i = 0; i < totalPeersInResponse; i++) {
            try {
                ...
                int port = (0xFF & (int) data.get()) << 8 | (0xFF & (int) data.get());
        ...
        return new UDPAnnounceResponseMessage(
                data, transactionId, interval, complete, incomplete, peers);
    }

    public static UDPAnnounceResponseMessage craft(
            int transactionId, int interval, int complete, int incomplete, List<Peer> peers) {
        ByteBuffer data =
                ByteBuffer.allocate(UDP_ANNOUNCE_RESPONSE_MIN_MESSAGE_SIZE + 6 * peers.size());
