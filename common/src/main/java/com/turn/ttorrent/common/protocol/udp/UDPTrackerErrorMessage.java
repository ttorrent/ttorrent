...
public class UDPTrackerErrorMessage extends UDPTrackerMessage.UDPTrackerResponseMessage
    ...
    public static UDPTrackerErrorMessage parse(ByteBuffer data) throws MessageValidationException {
        if (data.remaining() < UDP_TRACKER_ERROR_MIN_MESSAGE_SIZE) {
            throw new MessageValidationException("Invalid tracker error message size!");
        }

        if (data.getInt() != Type.ERROR.getId()) {
            throw new MessageValidationException("Invalid action code for tracker error!");
        }
        ...
        try {
            return new UDPTrackerErrorMessage(
                    data, transactionId, new String(reasonBytes, Constants.BYTE_ENCODING));
        } catch (UnsupportedEncodingException uee) {
            throw new MessageValidationException("Could not decode error message!", uee);
        }
    }

    public static UDPTrackerErrorMessage craft(int transactionId, String reason)
            throws UnsupportedEncodingException {
        byte[] reasonBytes = reason.getBytes(Constants.BYTE_ENCODING);
        ByteBuffer data =
                ByteBuffer.allocate(UDP_TRACKER_ERROR_MIN_MESSAGE_SIZE + reasonBytes.length);
