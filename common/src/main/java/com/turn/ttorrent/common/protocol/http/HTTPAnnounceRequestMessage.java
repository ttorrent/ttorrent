 ...
 *
 * <p>This class represents the announce request message in the HTTP tracker protocol. It doesn't
 * add any specific fields compared to the generic announce request message, but it provides the
 * means to parse such messages and craft them.
 *
 * @author mpetazzoni
 */
public class HTTPAnnounceRequestMessage extends HTTPTrackerMessage
    ...
    private HTTPAnnounceRequestMessage(
            ByteBuffer data,
            byte[] infoHash,
            Peer peer,
            long uploaded,
    ...
    public URL buildAnnounceURL(URL trackerAnnounceURL)
        ...
        url.append(base.contains("?") ? "&" : "?")
                ...
                .append(this.getPort())
                .append("&uploaded=")
                .append(this.getUploaded())
                .append("&downloaded=")
                .append(this.getDownloaded())
                .append("&left=")
                .append(this.getLeft())
                .append("&compact=")
                .append(this.isCompact() ? 1 : 0)
                .append("&no_peer_id=")
                .append(this.canOmitPeerId() ? 1 : 0);
    ...
    public static HTTPAnnounceRequestMessage parse(BEValue decoded)
        ...
        try {
            ...
            if (params.containsKey("event")) {
                event =
                        RequestEvent.getByName(
                                params.get("event").getString(Constants.BYTE_ENCODING));
            }
    ...
    public static HTTPAnnounceRequestMessage craft(
            byte[] infoHash,
            byte[] peerId,
            int port,
            long uploaded,
        ...
        if (event != null) {
            params.put("event", new BEValue(event.getEventName(), Constants.BYTE_ENCODING));
        }

        if (ip != null) {
            params.put("ip", new BEValue(ip, Constants.BYTE_ENCODING));
        }
