...
public class TorrentCreator {
     ...
     *
     * <p>Hash the given file to create the {@link TorrentMetadata} object representing the Torrent
     * meta info about this file, needed for announcing and/or sharing said file.
     *
     ...
     *
     * <p>Hash the given files to create the multi-file {@link TorrentMetadata} object representing
     * the Torrent meta-info about them, needed for announcing and/or sharing these files. Since we
     * created the torrent, we're considering we'll be a full initial seeder for it.
     *
     ...
     * @param createdBy The creator's name, or any string identifying the torrent's creator.
     */
    public static TorrentMetadata create(
            File parent, List<File> files, URI announce, String createdBy)
            throws InterruptedException, IOException {
     ...
     *
     * <p>Hash the given file to create the {@link TorrentMetadata} object representing the Torrent
     * metainfo about this file, needed for announcing and/or sharing said file.
     *
     * @param source The file to use in the torrent.
     ...
     */
    public static TorrentMetadata create(
            File source, List<List<URI>> announceList, String createdBy)
            throws InterruptedException, IOException {
     ...
     */
    public static TorrentMetadata create(
            File source, List<File> files, List<List<URI>> announceList, String createdBy)
            throws InterruptedException, IOException {
    ...
    static TorrentMetadata create(
        ...
        if (files == null || files.isEmpty()) {
            ...
            info.put(PIECES, new BEValue(hashFile(parent, pieceSize), Constants.BYTE_ENCODING));
        } else {
            ...
            info.put(PIECES, new BEValue(hashFiles(files, pieceSize), Constants.BYTE_ENCODING));
        }
     ...
     *
     * <p>Hashes the given file piece by piece using the default Torrent piece length (see {@link
     * #DEFAULT_PIECE_LENGTH}) and returns the concatenation of these hashes, as a string.
     *
     * <p>This is used for creating Torrent meta-info structures from a file.
     *
    ...
    private static String hashFiles(final List<File> files, final int pieceSize)
        ...
        int expectedPieces = (int) (Math.ceil((double) length / pieceSize));
        logger.debug(
                "Hashed {} file(s) ({} bytes) in {} pieces ({} expected) in {}ms.",
                new Object[] {
                    files.size(),
                    length,
                    results.size(),
    ...
    private static void waitForHashesToCalculate(List<Future<String>> results, StringBuilder hashes)
            throws InterruptedException, IOException {
        try {
            ...
            throw new RuntimeException(
                    String.format(
                            "very slow hashing: took more than %d seconds to calculate several"
                                + " pieces. Cancelling",
                            HASHING_TIMEOUT_SEC));
        }
    }
