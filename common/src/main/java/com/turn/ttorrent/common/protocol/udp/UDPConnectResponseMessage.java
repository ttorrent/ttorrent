...
public class UDPConnectResponseMessage extends UDPTrackerMessage.UDPTrackerResponseMessage
    ...
    public static UDPConnectResponseMessage parse(ByteBuffer data)
        ...
        if (data.remaining() != UDP_CONNECT_RESPONSE_MESSAGE_SIZE) {
            throw new MessageValidationException("Invalid connect response message size!");
        }

        if (data.getInt() != Type.CONNECT_RESPONSE.getId()) {
            throw new MessageValidationException("Invalid action code for connection response!");
        }

        return new UDPConnectResponseMessage(
                data,
    ...
    public static UDPConnectResponseMessage craft(int transactionId, long connectionId) {
        ByteBuffer data = ByteBuffer.allocate(UDP_CONNECT_RESPONSE_MESSAGE_SIZE);
