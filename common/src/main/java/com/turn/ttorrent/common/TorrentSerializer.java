...
public class TorrentSerializer {
    ...
    public byte[] serialize(TorrentMetadata metadata) throws IOException {
        ...
        if (metadata.getFiles().size() == 1) {
            ...
            for (TorrentFile torrentFile : metadata.getFiles()) {
                ...
                entry.put(
                        FILE_PATH,
                        new BEValue(mapStringListToBEValueList(torrentFile.relativePath)));
    ...
    @Nullable
    private List<BEValue> getAnnounceListAsBEValues(@Nullable List<List<String>> announceList)
    ...
    private List<BEValue> mapStringListToBEValueList(List<String> list)
            throws UnsupportedEncodingException {
        List<BEValue> result = new ArrayList<BEValue>();
        for (String s : list) {
    ...
    private void putOptionalIfPresent(
            Map<String, BEValue> map, String key, Optional<String> optional)
            throws UnsupportedEncodingException {
