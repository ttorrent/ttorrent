...
public class MetadataBuilder {
    ...
    // root dictionary
    @NotNull private String announce = "";
    @NotNull private List<List<String>> announceList = new ArrayList<List<String>>();
    private long creationDate = -1;
    @NotNull private String comment = "";
    @NotNull private String createdBy = DEFAULT_CREATED_BY;
    @NotNull private List<String> webSeedUrlList = new ArrayList<String>();
    // end root dictionary

    // info dictionary
    private int pieceLength = 512 * 1024; // 512kb by default
    private boolean isPrivate = false;
    @NotNull private List<String> filesPaths = new ArrayList<String>();
    @Nullable private HashingResult hashingResult = null;
    @NotNull private List<DataSourceHolder> dataSources = new ArrayList<DataSourceHolder>();
    @NotNull private String directoryName = "";
    // end info dictionary
    ...
    /**
     * set main announce tracker URL if you use single tracker. In case with many trackers use
     * {@link #addTracker(String)} and {@link #newTier()}. Then as main announce will be selected
     * first tracker. You can specify main announce using this method for override this behaviour
    ...
    /**
     * Multi-tracker Metadata Extension. Add new tracker URL to current tier. This method will
     * create first tier automatically if it doesn't exist You can find more information about this
     * extension in documentation <a
     * href="http://bittorrent.org/beps/bep_0012.html">http://bittorrent.org/beps/bep_0012.html</a>
     *
    ...
    /**
     * Multi-tracker Metadata Extension. Add all trackers to current tier. This method will create
     * first tier automatically if it doesn't exist You can find more information about this
     * extension in documentation <a
     * href="http://bittorrent.org/beps/bep_0012.html">http://bittorrent.org/beps/bep_0012.html</a>
     *
    ...
    /**
     * Multi-tracker Metadata Extension. Create new tier for adding tracker using {@link
     * #addTracker(String)} method If you don't add at least one tracker on the tier this tier will
     * be removed in building metadata You can find more information about this extension in
     * documentation <a
     * href="http://bittorrent.org/beps/bep_0012.html">http://bittorrent.org/beps/bep_0012.html</a>
     */
    ...
    /**
     * Web Seeding Metadata. Web seeding url as defined by <a
     * href='http://bittorrent.org/beps/bep_0019.html'>bep 0019</a>
     *
     ...
     */
    public MetadataBuilder setPiecesHashesCalculator(
            @NotNull PiecesHashesCalculator piecesHashesCalculator) {
        this.piecesHashesCalculator = piecesHashesCalculator;
        return this;
    }

    /**
     * Set length int bytes of one piece. By default is used 512KB. Larger piece size reduces size
     * of .torrent file but cause inefficiency (torrent-client need to download full piece from peer
     * for validating) and too-small piece sizes cause large .torrent metadata file. Recommended
     * size is between 256KB and 1MB.
     */
    ...
    /**
     * Set the name of the directory in which to store all the files. If {@link #directoryName}
     * isn't empty then multi-file torrent will be created, otherwise single-file
     */
     ...
     *
     * @param closeAfterBuild if true then source stream will be closed after {@link #build()}
     *     invocation
     */
    public MetadataBuilder addDataSource(
            @NotNull InputStream dataSource, String path, boolean closeAfterBuild) {
    ...
    /**
     * add specified file in torrent with custom path. The file will be stored in .torrent by
     * specified path. Path can be separated with any slash. In case of single-file torrent this
     * path will be used as name of source file
     */
    ...
    private void checkHashingResultIsNotSet() {
        if (hashingResult != null) {
            throw new IllegalStateException(
                    "Unable to add new source when hashes are set manually");
        }
    }

    /**
     * add specified file in torrent. In case of multi-torrent this file will be downloaded to
     * {@link #directoryName}. In single-file torrent this file will be downloaded in download
     * folder
     */
     ...
     * Using of this method is not compatible with using source-based methods ({@link
     * #addFile(File)}, {@link #addDataSource(InputStream, String, boolean)}, etc because it's not
     * possible to calculate concat this hashes and calculated hashes. each byte array in hashes
     * list should have {{@link Constants#PIECE_HASH_SIZE}} length
     *
     ...
     */
    public MetadataBuilder setFilesInfo(
            @NotNull List<byte[]> hashes,
            @NotNull List<String> filesPaths,
            @NotNull List<Long> filesLengths) {
        if (dataSources.size() != 0) {
            throw new IllegalStateException(
                    "Unable to add hashes-based files info. Some data sources already added");
        }
     ...
     *
     * @see <a
     *     href="http://bittorrent.org/beps/bep_0027.html">http://bittorrent.org/beps/bep_0027.html</a>
     */
    ...
    private BEValue doBuild() throws IOException {
        ...
        if (filesPaths.size() == 0) {
            throw new IllegalStateException(
                    "Unable to create metadata without sources. Use addSource() method for adding"
                        + " sources");
        }
        ...
        if (!directoryName.isEmpty()) {
            ...
            if (isSingleMode) {
                ...
                throw new IllegalStateException(
                        "Missing required field 'name'. Use setDirectoryName() method for"
                            + " specifying name of torrent");
            }
        }
    ...
    private List<BEValue> getFilesList(HashingResult hashingResult)
    ...
    private static class FileSourceHolder implements DataSourceHolder {
        ...
        @NotNull private final File source;
