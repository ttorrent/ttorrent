...
public class TorrentParser {
     ...
     * @return parsed metadata object. This parser also wraps single torrent as multi torrent with
     *     one file
     * @throws InvalidBEncodingException if metadata has incorrect BEP format or missing required
     *     fields
     * @throws RuntimeException It's wrapped io exception from bep decoder. This exception doesn't
     *     must to throw io exception because reading from byte array input stream cannot throw the
     *     exception
     */
    public TorrentMetadata parse(byte[] metadata)
            throws InvalidBEncodingException, RuntimeException {
        ...
        final Map<String, BEValue> infoTable =
                getRequiredValueOrThrowException(dictionaryMetadata, INFO_TABLE).getMap();
    ...
    private List<TorrentFile> parseFiles(
            Map<String, BEValue> infoTable, boolean torrentContainsManyFiles, String name)
    ...
    @Nullable
    private String getStringOrNull(Map<String, BEValue> dictionaryMetadata, String key)
            throws InvalidBEncodingException {
    ...
    @Nullable
    private List<List<String>> getTrackers(Map<String, BEValue> dictionaryMetadata)
            throws InvalidBEncodingException {
    ...
    @NotNull
    private BEValue getRequiredValueOrThrowException(Map<String, BEValue> map, String key)
            throws InvalidBEncodingException {
        final BEValue value = map.get(key);
        if (value == null)
            throw new InvalidBEncodingException(
                    "Invalid metadata format. Map doesn't contain required field " + key);
        return value;
    }
}
