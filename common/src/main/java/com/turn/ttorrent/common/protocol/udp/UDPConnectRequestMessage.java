 ...
 * @author mpetazzoni
 */
public class UDPConnectRequestMessage extends UDPTrackerMessage.UDPTrackerRequestMessage
    ...
    public static UDPConnectRequestMessage parse(ByteBuffer data)
        ...
        if (data.remaining() != UDP_CONNECT_REQUEST_MESSAGE_SIZE) {
            throw new MessageValidationException("Invalid connect request message size!");
        }

        if (data.getLong() != UDP_CONNECT_REQUEST_MAGIC) {
            throw new MessageValidationException("Invalid connection ID in connection request!");
        }

        if (data.getInt() != Type.CONNECT_REQUEST.getId()) {
            throw new MessageValidationException("Invalid action code for connection request!");
        }

        return new UDPConnectRequestMessage(
                data, data.getInt() // transactionId
                );
    }
