 ...
 * <p>
 *
 * <p>A BitTorrent client in its bare essence shares a given torrent. If the torrent is not complete
 ...
 * <p>
 *
 * <p>This BitTorrent client implementation is made to be simple to embed and simple to use. First,
 * initialize a ShareTorrent object from a torrent meta-info source (either a file or a byte array,
 * see com.turn.ttorrent.SharedTorrent for how to create a SharedTorrent object).
 *
 * @author mpetazzoni
 */
public class CommunicationManager
    ...
    protected static final Logger logger =
            TorrentLoggerFactory.getLogger(CommunicationManager.class);
    ...
    /**
     * @param workingExecutor executor service for run connection worker and process incoming data.
     *     Must have a pool size at least 2
     * @param pieceValidatorExecutor executor service for calculation sha1 hashes of downloaded
     *     pieces
     */
    public CommunicationManager(
            ExecutorService workingExecutor, ExecutorService pieceValidatorExecutor) {
    ...
    public CommunicationManager(
            ...
            ExecutorService pieceValidatorExecutor,
     ...
     */
    public TorrentManager addTorrent(String dotTorrentFilePath, String downloadDirPath)
            throws IOException {
        return addTorrent(dotTorrentFilePath, downloadDirPath, FairPieceStorageFactory.INSTANCE);
    }

    /**
     * Adds torrent to storage with specified listeners, validate downloaded files and start seeding
     * and leeching the torrent
     *
     * @param dotTorrentFilePath path to torrent metadata file
     * @param downloadDirPath path to directory where downloaded files are placed
     ...
     * @throws IOException if IO error occurs in reading metadata file
     */
    public TorrentManager addTorrent(
            String dotTorrentFilePath, String downloadDirPath, List<TorrentListener> listeners)
            throws IOException {
        return addTorrent(
                dotTorrentFilePath, downloadDirPath, FairPieceStorageFactory.INSTANCE, listeners);
    }

    /**
     * Adds torrent to storage with specified {@link PieceStorageFactory}. It can be used for
     * skipping initial validation of data
     *
     * @param dotTorrentFilePath path to torrent metadata file
     * @param downloadDirPath path to directory where downloaded files are placed
    ...
    public TorrentManager addTorrent(
        ...
        return addTorrent(
                ...
                downloadDirPath,
                pieceStorageFactory,
                Collections.<TorrentListener>emptyList());
    }
    ...
    public TorrentManager addTorrent(
            ...
            String downloadDirPath,
            PieceStorageFactory pieceStorageFactory,
            List<TorrentListener> listeners)
            throws IOException {
        ...
        FileCollectionStorage fileCollectionStorage =
                FileCollectionStorage.create(metadata, new File(downloadDirPath));
        PieceStorage pieceStorage =
                pieceStorageFactory.createStorage(metadata, fileCollectionStorage);
     ...
     */
    public TorrentManager addTorrent(
            TorrentMetadataProvider metadataProvider, PieceStorage pieceStorage)
            throws IOException {
    ...
    public TorrentManager addTorrent(
        ...
        final LoadedTorrentImpl loadedTorrent =
                new LoadedTorrentImpl(
        ...
        this.torrentsStorage.addTorrent(
                loadedTorrent.getTorrentHash().getHexInfoHash(), loadedTorrent);
        forceAnnounceAndLogError(loadedTorrent, pieceStorage.isFinished() ? COMPLETED : STARTED);
        logger.debug(
                String.format(
                        "Added torrent %s (%s)",
                        loadedTorrent, loadedTorrent.getTorrentHash().getHexInfoHash()));
    ...
    private void forceAnnounceAndLogError(
            LoadedTorrent torrent, AnnounceRequestMessage.RequestEvent event) {
        try {
    ...
    private void sendStopEvent(LoadedTorrent loadedTorrent, String torrentHash) {
        if (loadedTorrent == null) {
            logger.info(
                    "Announceable torrent {} not found in storage after unsuccessful download"
                        + " attempt",
                    torrentHash);
            return;
        }
    ...
    /**
     * set ups new receive buffer size, that will be applied to all new connections. If value is
     * equal or less, than zero, then method doesn't have effect
     *
    ...
    /**
     * set ups new send buffer size, that will be applied to all new connections. If value is equal
     * or less, than zero, then method doesn't have effect
     *
     * @param newSize new size
     */
    ...
    /**
     * Runs client instance and starts announcing, seeding and downloading of all torrents from
     * storage
     *
     * @param bindAddresses list of addresses which are used for sending to the tracker. Current
     *     client must be available for other peers on the addresses
     ...
     * @param bindAddresses list of addresses which are used for sending to the tracker. Current
     *     client must be available for other peers on the addresses
     * @param defaultTrackerURI default tracker address. All torrents will be announced not only on
     *     the trackers from metadata file but also to this tracker
     * @throws IOException if any io error occurs
     */
    public void start(final InetAddress[] bindAddresses, final URI defaultTrackerURI)
            throws IOException {
        start(
                bindAddresses,
                Constants.DEFAULT_ANNOUNCE_INTERVAL_SEC,
    ...
    public Peer[] getSelfPeers(final InetAddress[] bindAddresses)
        ...
        for (int i = 0; i < bindAddresses.length; i++) {
            ...
            final Peer peer =
                    new Peer(new InetSocketAddress(bindAddress.getHostAddress(), self.getPort()));
            peer.setTorrentHash(self.getHexInfoHash());
            // if we have more, that one bind address, then only for first set self peer id. For
            // other generate it
            if (i == 0) {
                peer.setPeerId(self.getPeerId());
            } else {
                final String id =
                        CommunicationManager.BITTORRENT_ID_PREFIX
     ...
     * @param bindAddresses list of addresses which are used for sending to the tracker. Current
     *     client must be available for other peers on the addresses
     * @param announceIntervalSec default announce interval. This interval can be override by
     *     tracker
     * @param defaultTrackerURI default tracker address. All torrents will be announced not only on
     *     the trackers from metadata file but also to this tracker
     ...
     */
    public void start(
            final InetAddress[] bindAddresses,
    ...
    public void start(
            ...
            final URI defaultTrackerURI,
            final SelectorFactory selectorFactory,
            final ServerChannelRegister serverChannelRegister)
            throws IOException {
        ...
        this.setSocketConnectionTimeout(
                DEFAULT_SOCKET_CONNECTION_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS);
        try {
        ...
        Peer self =
                new Peer(
                        new InetSocketAddress(myConnectionManager.getBindPort()),
                        ByteBuffer.wrap(idBytes));
        peersStorage.setSelf(self);
        logger.info(
                "BitTorrent client [{}] started and " + "listening at {}:{}...",
                new Object[] {self.getShortHexPeerId(), self.getIp(), self.getPort()});
    ...
    @Override
    public SharingPeer createSharingPeer(
            String host,
            int port,
            ByteBuffer peerId,
     ...
     */
    @Override
    public void handleAnnounceResponse(
            int interval, int complete, int incomplete, String hexInfoHash) {
    ...
    public void handleDiscoveredPeers(List<Peer> peers, String hexInfoHash) {
        ...
        if (announceableTorrent == null) {
            logger.info(
                    "announceable torrent {} is not found in storage. Maybe it was removed",
                    hexInfoHash);
            return;
        }
        ...
        logger.debug(
                "Got {} peer(s) ({}) for {} in tracker response",
                new Object[] {peers.size(), Arrays.toString(peers.toArray()), hexInfoHash});
        ...
        for (Map.Entry<PeerUID, Peer> e : uniquePeers.entrySet()) {
            ...
            ConnectionListener connectionListener =
                    new OutgoingConnectionListener(
                            this,
            ...
            boolean connectTaskAdded =
                    this.myConnectionManager.offerConnect(
                            new ConnectTask(
                                    peer.getIp(),
                                    peer.getPort(),
                                    connectionListener,
                                    Clock.systemUTC().millis(),
                                    Constants.DEFAULT_CONNECTION_TIMEOUT_MILLIS),
                            1,
                            TimeUnit.SECONDS);
            if (!connectTaskAdded) {
                logger.info(
                        "can not connect to peer {}. Unable to add connect task to connection"
                            + " manager",
                        peer);
            }
        }
     ...
     * <p>
     *
     * <p>We use this handler to identify when all of the pieces have been downloaded. When that's
     * the case, we can start the seeding period, if any.
     *
    ...
    public void handlePieceCompleted(final SharingPeer peer, final Piece piece) throws IOException {
        ...
        try {
            final Future<?> validationFuture =
                    myPieceValidatorExecutor.submit(
                            new Runnable() {
                                @Override
                                public void run() {
            ...
            LoggerUtils.warnWithMessageAndDebugDetails(
                    logger, "Unable to submit validation task for torrent {}", torrentHash, e);
        }
    }

    private void validatePieceAsync(
            final SharedTorrent torrent, final Piece piece, String torrentHash, SharingPeer peer) {
        try {
            synchronized (piece) {
                ...
                if (piece.isValid()) {
                    ...
                    synchronized (torrent) {
                        ...
                        boolean isCurrentPeerSeeder =
                                peer.getAvailablePieces().cardinality() == torrent.getPieceCount();
                        // if it's seeder we will send not interested message when we download full
                        // file
                    ...
                    if (isTorrentComplete) {
                        ...
                        LoadedTorrent announceableTorrent =
                                torrentsStorage.getLoadedTorrent(torrentHash);

                        if (announceableTorrent == null) return;

                        AnnounceableInformation announceableInformation =
                                announceableTorrent.createAnnounceableInformation();

                        if (!TorrentUtils.isTrackerLessInfo(announceableInformation)) {
                            try {
                                announce.getCurrentTrackerClient(announceableInformation)
                                        .announceAllInterfaces(
                                                COMPLETED, true, announceableInformation);
                            } catch (AnnounceException e) {
                                logger.debug(
                                        "unable to announce torrent {} on tracker {}",
                                        torrent,
                                        torrent.getAnnounce());
                            }
                        }
                    ...
                    torrent.markUncompleted(piece);
                    logger.info(
                            "Downloaded piece #{} from {} was not valid ;-(. Trying another peer",
                            piece.getIndex(),
                            peer);
    ...
    public void handlePeerDisconnected(SharingPeer peer) {
        ...
        logger.trace(
                "Peer {} disconnected, [{}/{}].",
                new Object[] {
                    peer, getConnectedPeers().size(), this.peersStorage.getSharingPeers().size()
                });
    ...
    public void handleIOException(SharingPeer peer, IOException ioe) {
        logger.debug(
                "I/O problem occured when reading or writing piece data for peer {}: {}.",
                peer,
                ioe.getMessage());
