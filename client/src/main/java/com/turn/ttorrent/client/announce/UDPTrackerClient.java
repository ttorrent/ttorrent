 ...
 *
 * <p>The UDP tracker protocol requires a two-step announce request/response exchange where the peer
 * is first required to establish a "connection" with the tracker by sending a connection request
 ...
 *
 * <p>It also contains a backing-off retry mechanism (on a 15*2^n seconds scheme), in which if the
...
public class UDPTrackerClient extends TrackerClient {
    ...
    protected static final Logger logger = TorrentLoggerFactory.getLogger(UDPTrackerClient.class);
    ...
    /**
     * For STOPPED announce event, we don't want to be bothered with waiting that long. We'll try
     * once and bail-out early.
     */
     ...
     *
     * <p>The biggest packet in the exchange is the announce response, which in 20 bytes + 6 bytes
     * per peer. Common numWant is 50, so 20 + 6 * 50 = 320. With headroom, we'll ask for 512 bytes.
     */
    ...
    /** */
    protected UDPTrackerClient(List<Peer> peers, URI tracker) throws UnknownHostException {
        super(peers, tracker);
    ...
    protected void multiAnnounce(
            ...
            List<? extends AnnounceableInformation> torrents,
            List<Peer> peer)
    ...
    public void announce(
            ...
            final AnnounceableInformation torrent,
        ...
        try {
            ...
            while (++attempts <= maxAttempts) {
                ...
                if (this.connectionExpiration != null) {
                    if (new Date().before(this.connectionExpiration)) {
                        ...
                        logger.debug(
                                "Announce connection ID expired, "
                    ...
                    case CONNECT_REQUEST:
                        ...
                        try {
                            this.handleTrackerConnectResponse(
                                    UDPTrackerMessage.UDPTrackerResponseMessage.parse(
                                            this.recv(attempts)));
                            attempts = -1;
            ...
            throw new AnnounceException(
                    "Tracker message violates expected " + "protocol (" + mve.getMessage() + ")",
                    mve);
        }
    }
     ...
     *
     * <p>Verifies the transaction ID of the message before passing it over to {@link Announce#()}.
     *
    ...
    protected void handleTrackerAnnounceResponse(
            TrackerMessage message, boolean inhibitEvents, String hexInfoHash)
            throws AnnounceException {
     ...
     *
     * <p>Verifies that the message is not an error message (throws an exception with the error
     * message if it is) and that the transaction ID matches the current one.
     *
    ...
    private void handleTrackerConnectResponse(TrackerMessage message) throws AnnounceException {
        ...
        if (!(message instanceof ConnectionResponseMessage)) {
            throw new AnnounceException(
                    "Unexpected tracker message type " + message.getType().name() + "!");
        }
    ...
    private void send(ByteBuffer data) {
        try {
            ...
            logger.info(
                    "Error sending datagram packet to tracker at {}: {}.",
                    this.address,
                    ioe.getMessage());
        }
    }
    ...
    private ByteBuffer recv(int attempt)
        ...
        logger.trace("Setting receive timeout to {}s for attempt {}...", timeout, attempt);
        this.socket.setSoTimeout(timeout * 1000);

        try {
            DatagramPacket p = new DatagramPacket(new byte[UDP_PACKET_LENGTH], UDP_PACKET_LENGTH);
