 ...
 * A torrent piece.
 *
 * <p>This class represents a torrent piece. Torrents are made of pieces, which are in turn made of
 ...
 *
 * <p>If the torrent has multiple files, pieces can spread across file boundaries. The
...
public class Piece implements Comparable<Piece>, PieceInformation {
     ...
     *
     * <p>All pieces, except the last one, are expected to have the same size.
     */
    public long size() {
        return this.length;
    }
     ...
     *
     * <p>This function will read the piece data without checking if the piece has been validated.
     * It is simply meant at factoring-in the common read code from the validate and read functions.
     *
     ...
     * @throws IOException If the read can't be completed (I/O error, or EOF reached, which can
     *     happen if the piece is not complete).
     */
     ...
     *
     * <p>This is the public method for reading this piece's data, and it will only succeed if the
     * piece is complete and valid on disk, thus ensuring any data that comes out of this function
     * is valid piece data we can send to other peers.
     *
     * @param offset Offset inside this piece where to start reading.
     * @param length Number of bytes to read from the piece.
     * @return A byte buffer containing the piece data.
    ...
    public ByteBuffer read(long offset, int length, ByteBuffer block)
        ...
        if (!this.valid) {
            throw new IllegalStateException(
                    "Attempting to read an " + "known-to-be invalid piece!");
        }
