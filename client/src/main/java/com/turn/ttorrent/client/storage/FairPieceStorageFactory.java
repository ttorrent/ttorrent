...
/**
 * This implementation will read all pieces from storage and compare hashes of pieces with really
 * hashed from metadata
 */
            ...
            if (!byteStorage.isBlank()) {
                ...
                for (int i = 0; i < metadata.getPiecesCount(); i++) {
                    ...
                    if (!byteStorage.isBlank(position, len)) {
                        ...
                        byte[] expectedHash =
                                Arrays.copyOfRange(
                                        metadata.getPiecesHashes(),
                                        i * Constants.PIECE_HASH_SIZE,
                                        (i + 1) * Constants.PIECE_HASH_SIZE);
