    ...
    public DataProcessor processAndGetNext(ByteChannel socketChannel) throws IOException {
        ...
        try {
            ...
            LoggerUtils.warnAndDebugDetails(
                    logger, "unable to read data from {}", socketChannel, e);
        }
        if (readBytes == -1) {
            return new ShutdownProcessor().processAndGetNext(socketChannel);
        }
        ...
        final LoadedTorrent announceableTorrent =
                myContext.getTorrentsStorage().getLoadedTorrent(hs.getHexInfoHash());
        ...
        try {
            ...
            LoggerUtils.warnWithMessageAndDebugDetails(
                    logger, "cannot load torrent {}", hs.getHexInfoHash(), e);
            return new ShutdownProcessor().processAndGetNext(socketChannel);
        }

        logger.trace(
                "got handshake {} from {}", Arrays.toString(messageBytes.array()), socketChannel);
        ...
        if (old != null) {
            logger.debug(
                    "Already connected to old peer {}, close current connection with {}",
                    old,
                    sharingPeer);
            return new ShutdownProcessor().processAndGetNext(socketChannel);
        }
        ...
        if (!myIsOutgoingConnection) {
            ...
            try {
                final Handshake craft =
                        Handshake.craft(
                                hs.getInfoHash(),
                                myContext.getPeersStorage().getSelf().getPeerIdArray());
                socketChannel.write(craft.getData());
            } catch (IOException e) {
                LoggerUtils.warnAndDebugDetails(
                        logger, "error in sending handshake to {}", socketChannel, e);
        ...
        try {
            ...
            LoggerUtils.warnAndDebugDetails(
                    logger,
                    "task 'onConnectionEstablished' submit is failed. Reason: {}",
                    e.getMessage(),
                    e);
            return new ShutdownAndRemovePeerProcessor(peerUID, myContext)
