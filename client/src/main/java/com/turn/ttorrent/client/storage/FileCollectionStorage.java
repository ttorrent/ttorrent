 ...
 *
 * <p>This implementation of the torrent byte storage provides support for multi-file torrents and
 * completely abstracts the read/write operations from the notion of different files. The byte
 * storage is represented as one continuous byte storage, directly accessible by offset regardless
 * of which file this offset lands.
 *
 * @author mpetazzoni
 * @author dgiffin
 */
public class FileCollectionStorage implements TorrentByteStorage {
     ...
     *
     * @param files The list of individual {@link FileStorage} objects making up the torrent.
    ...
    public FileCollectionStorage(List<FileStorage> files, long size) {
        ...
        logger.debug(
                "Initialized torrent byte storage on {} file(s) " + "({} total byte(s)).",
                files.size(),
                size);
    }

    public static FileCollectionStorage create(TorrentMetadata metadata, File parent)
            throws IOException {
        ...
        for (TorrentFile file : metadata.getFiles()) {
            ...
            if (!actual.getCanonicalPath().startsWith(parent.getCanonicalPath())) {
                throw new SecurityException(
                        "Torrent file path attempted " + "to break directory jail!");
            }
    ...
    public synchronized void open(final boolean seeder) throws IOException {
        for (FileStorage file : files) {
            if (!file.isOpen()) file.open(seeder);
        }
        myIsOpen = true;
    }
     ...
     *
     * <p>This simple inner class holds the details for a read or write operation on one of the
     * underlying {@link FileStorage}s.
     *
     * @author dgiffin
     * @author mpetazzoni
     */
     ...
     *
     * <p>This function selects which files are impacted by a read or write operation, with their
     * respective relative offset and chunk length.
     *
     * @param offset The offset of the operation, in bytes, relative to the complete byte storage.
     * @param length The number of bytes to read or write.
     * @return A list of {@link FileOffset} objects representing the {@link FileStorage}s impacted
     *     by the operation, bundled with their respective relative offset and number of bytes to
     *     read or write.
     * @throws IllegalArgumentException If the offset and length go over the byte storage size.
     * @throws IllegalStateException If the files registered with this byte storage can't
     *     accommodate the request (should not happen, really).
     */
    private List<FileOffset> select(long offset, long length) {
        if (offset + length > this.size) {
            throw new IllegalArgumentException(
                    "Buffer overrun (" + offset + " + " + length + " > " + this.size + ") !");
        }
