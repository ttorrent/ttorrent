...
public class HTTPTrackerClient extends TrackerClient {
     ...
     *
     * <p>This function first builds an announce request for the specified event with all the
     * required parameters. Then, the request is made to the tracker and the response analyzed.
     *
    ...
    public void announce(
            ...
            final AnnounceableInformation torrentInfo,
            final List<Peer> adresses)
        ...
        for (final Peer address : adresses) {
            ...
            try {
                sendAnnounce(
                        ...
                        new ResponseParser() {
                            @Override
                            public void parse(InputStream inputStream, int responseCode)
                                    throws IOException, MessageValidationException {
                                if (responseCode != 200) {
                                    logger.info(
                                            "received not http 200 code from tracker for request "
                                                    + target);
                                    return;
                                }
    ...
    protected void multiAnnounce(
        ...
        for (final Peer address : addresses) {
            ...
            final List<HTTPTrackerMessage> responsesForCurrentIp =
                    new ArrayList<HTTPTrackerMessage>();
            ...
            sendAnnounce(
                    ...
                    new ResponseParser() {
                        ...
                        public void parse(InputStream inputStream, int responseCode)
                            ...
                            if (responseCode != 200) {
                                logger.info(
                                        "received {} code from tracker for multi announce request.",
                                        responseCode);
                            ...
                            if (bdecode == null) {
                                logger.info(
                                        "tracker sent bad response for multi announce message.");
                                logger.debug(bodyStr);
                                return;
                            }
            ...
            for (HTTPTrackerMessage message : messages) {
                ...
                try {
                    ...
                    LoggerUtils.errorAndDebugDetails(
                            logger, "Unable to process tracker response {}", message, e);
                }
            }
        }
    }

    private URL encodeAnnounceToURL(
            AnnounceRequestMessage.RequestEvent event,
            AnnounceableInformation torrentInfo,
            Peer peer)
            throws AnnounceException {
        URL result;
        try {
            ...
            throw new AnnounceException(
                    "Announce request creation violated "
                            + "expected protocol ("
                            + mve.getMessage()
                            + ")",
                    mve);
    ...
    private void sendAnnounce(
            final URL url, final String body, final String method, ResponseParser parser)
            throws AnnounceException, ConnectException {
    ...
    private URLConnection openConnectionCheckRedirects(URL url, String body, String method)
            throws IOException {
    ...
    private interface ResponseParser {
        ...
        void parse(InputStream inputStream, int responseCode)
                throws IOException, MessageValidationException;
    }
}
