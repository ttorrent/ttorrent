...
public class SimpleClient {
    ...
    public SimpleClient(int workingExecutorSize, int validatorExecutorSize) {
        communicationManager =
                new CommunicationManager(
                        Executors.newFixedThreadPool(workingExecutorSize),
                        Executors.newFixedThreadPool(validatorExecutorSize));
    }
    ...
    public void stop(int timeout, TimeUnit timeUnit) {
        ...
        for (ExecutorService executorService :
                Arrays.asList(
        ...
        if (interruptedException != null) {
            throw new RuntimeException(
                    "Thread was interrupted, "
                            + "shutdown methods are invoked but maybe tasks are not finished yet",
                    interruptedException);
        }
        if (anyFailedByTimeout)
            throw new RuntimeException(
                    "At least one executor was not fully shutdown because timeout was elapsed");
    }

    public void downloadTorrent(String torrentFile, String downloadDir, InetAddress iPv4Address)
            throws IOException, InterruptedException {
        communicationManager.start(iPv4Address);
        final Semaphore semaphore = new Semaphore(0);
        List<TorrentListener> listeners =
                Collections.<TorrentListener>singletonList(
        ...
        TorrentManager torrentManager =
                communicationManager.addTorrent(torrentFile, downloadDir, listeners);
        semaphore.acquire();
    }

    private TorrentManager startDownloading(
            String torrentFile, String downloadDir, InetAddress iPv4Address) throws IOException {
        communicationManager.start(iPv4Address);
        return communicationManager.addTorrent(torrentFile, downloadDir);
    }

    public TorrentManager downloadTorrentAsync(
            String torrentFile, String downloadDir, InetAddress iPv4Address) throws IOException {
    ...
    public TorrentStatistic getStatistics(String dotTorrentFilePath) throws IOException {
        ...
        LoadedTorrent loadedTorrent =
                communicationManager
                        .getTorrentsStorage()
                        .getLoadedTorrent(metadata.getHexInfoHash());
