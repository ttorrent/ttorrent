 ...
 *
 * <p>This implementation of TorrentByteStorageFile provides a torrent byte data storage relying on
 * a single underlying file and uses a RandomAccessFile FileChannel to expose thread-safe read/write
 * methods.
 *
 * @author mpetazzoni
 */
public class FileStorage implements TorrentByteStorage {
    ...
    public void open(final boolean seeder) throws IOException {
        try {
            ...
            if (seeder) {
                if (!target.exists()) {
                    throw new IOException(
                            "Target file " + target.getAbsolutePath() + " doesn't exist.");
                }
                ...
                if (this.partial.exists()) {
                    logger.debug(
                            "Partial download found at {}. Continuing...",
            ...
            logger.debug(
                    "Opened byte storage file at {} ({}+{} byte(s)).",
                    new Object[] {
                        this.current.getAbsolutePath(), this.offset, this.size,
                    });
        } finally {
    ...
    public void close() throws IOException {
        try {
            ...
            if (!myIsOpen) return;
            logger.debug(
                    "Closing file channel to {}. Channel open: {}",
                    current.getName(),
                    channel.isOpen());
    ...
    public void finish() throws IOException {
        try {
            myLock.writeLock().lock();
            logger.debug(
                    "Closing file channel to " + this.current.getName() + " (download complete).");
            if (this.channel.isOpen()) {
                this.channel.force(true);
            }
            ...
            try {
                ...
                if (this.target.exists()) {
                    throw new IOException(
                            "Was unable to delete existing file " + target.getAbsolutePath(), ex);
                }
