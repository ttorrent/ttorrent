 ...
 * <p>
 *
 * <p>A SharingPeer extends the base Peer class with all the data and logic needed by the BitTorrent
 * client to interact with a peer exchanging on the same torrent.
 *
 * <p>
 *
 * <p>Peers are defined by their peer ID, IP address and port number, just like base peers. Peers we
 * exchange with also contain four crucial attributes:
 *
 * <p>
 *
 * <ul>
 *   <li><code>choking</code>, which means we are choking this peer and we're not willing to send
 *       him anything for now;
 *   <li><code>interesting</code>, which means we are interested in a piece this peer has;
 *   <li><code>choked</code>, if this peer is choking and won't send us anything right now;
 *   <li><code>interested</code>, if this peer is interested in something we have.
 * </ul>
 *
 * <p>
 *
 * <p>Peers start choked and uninterested.
 *
 * @author mpetazzoni
 */
public class SharingPeer extends Peer implements MessageListener, PeerInformation {
     ...
     * <p>
     *
     * <p>We don't want to upload to this peer anymore, so mark that we're choking from this peer.
     */
    public void choke() {
        if (!this.choking) {
     ...
     * <p>
     *
     * <p>Mark that we are no longer choking from this peer and can resume uploading to it.
     */
    public void unchoke() {
     ...
     * <p>
     *
     * <p>This terminates the eventually present and/or connected peer exchange with the peer and
     * fires the peer disconnected event to any peer activity listeners registered on this peer.
     *
    ...
    public void unbind(boolean force) {
        ...
        try {
            ...
            LoggerUtils.errorAndDebugDetails(
                    logger, "cannot close socket channel. Peer {}", this, e);
        }
     ...
     * <p>
     *
     * <p>Delivery of the message can only happen if the peer is connected.
     *
     ...
     * <p>
     *
     * <p>Starts a block request queue and pre-fill it with MAX_PIPELINED_REQUESTS block requests.
     *
     * <p>
     *
     * <p>Further requests will be added, one by one, every time a block is returned.
     *
     * @param piece The piece chosen to be downloaded from this peer.
     */
    public void downloadPiece(final Piece piece) throws IllegalStateException {
        ...
        synchronized (this.requestsLock) {
            ...
            while (lastRequestedOffset < piece.size()) {
                PeerMessage.RequestMessage request =
                        PeerMessage.RequestMessage.craft(
                                piece.getIndex(),
                                lastRequestedOffset,
                                Math.min(
                                        (int) (piece.size() - lastRequestedOffset),
     ...
     * <p>
     *
     * <p>Upon reception of a piece block with a PIECE message, remove the corresponding request
     * from the pipeline to make room for the next block requests.
     *
     ...
     * <p>
     *
     * <p>This queues CANCEL messages for all the requests in the queue, and returns the list of
     * requests that were in the queue.
     *
     * <p>
     *
     * <p>If no request queue existed, or if it was empty, an empty set of request messages is
     * returned.
     */
        ...
        switch (msg.getType()) {
            ...
            case BITFIELD:
                ...
                synchronized (this.availablePiecesLock) {
                    ...
                    logger.trace(
                            "Recorded bitfield from {} with {} " + "pieces(s) [{}/{}].",
                            new Object[] {
                                this,
            ...
            case REQUEST:
                ...
                if (!rp.isValid()) {
                    logger.warn(
                            "Peer {} violated protocol, terminating exchange: "
                                    + this.isChoking()
                                    + " "
                                    + rp.isValid(),
                            this);
                ...
                try {
                    ...
                    ByteBuffer bufferForMessage =
                            PeerMessage.PieceMessage.createBufferWithHeaderForMessage(
                                    request.getPiece(), request.getOffset(), request.getLength());

                    rp.read(request.getOffset(), request.getLength(), bufferForMessage);

                    this.send(
                            PeerMessage.PieceMessage.craft(
                                    request.getPiece(), request.getOffset(), bufferForMessage));
                    ...
                    this.fireIOException(
                            new IOException("Error while sending piece block request!", ioe));
                }
            ...
            case PIECE:
                ...
                logger.trace(
                        "Got piece ({} {}@{}) from {}",
                        new Object[] {p.getIndex(), p.size(), piece.getOffset(), this});
                ...
                try {
                    ...
                    this.fireIOException(
                            new IOException("Error while storing received piece block!", ioe));
                    break;
                }
                break;
            case CANCEL:
     ...
     * <p>
     *
     * <p>The event contains the peer that chocked.
     */
     ...
     * <p>
     *
     * <p>The event contains the peer that unchoked or became ready.
     */
     ...
     * <p>
     *
     * <p>The event contains the peer (this), and the piece that became available.
     */
     ...
     *
     * <p>The event contains the peer (this), and the bit field of available pieces from this peer.
     */
     ...
     * <p>
     *
     * <p>The event contains the peer (this), and the piece number that was sent to the peer.
     *
     ...
     * <p>
     *
     * <p>The event contains the peer (this), and the piece number that was completed.
     *
     * @param piece The completed piece.
     */
     ...
     * <p>
     *
     * <p>The event contains the peer that disconnected (this).
     */
     ...
     * <p>
     *
     * <p>The event contains the peer that triggered the problem, and the exception object.
     */
     ...
     * <p>
     *
     * <p>Compares sharing peers based on their current download rate.
     *
     * @author mpetazzoni
