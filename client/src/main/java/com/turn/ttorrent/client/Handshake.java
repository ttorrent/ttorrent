...
public class Handshake implements TorrentHash {
    ...
    public static Handshake parse(ByteBuffer buffer)
        ...
        if (pstrlen < 0 || buffer.remaining() != BASE_HANDSHAKE_LENGTH + pstrlen - 1) {
            throw new ParseException(
                    "Incorrect handshake message length " + "(pstrlen=" + pstrlen + ") !", 0);
        }
    ...
    public static Handshake parse(ByteBuffer buffer, String torrentIdentifier)
    ...
    public static Handshake parse(ByteBuffer buffer, int pstrlen)
            throws UnsupportedEncodingException, ParseException {
        Handshake hs = Handshake.parse(buffer);
    ...
    public static Handshake craft(byte[] torrentInfoHash, byte[] clientPeerId) {
        try {
            ...
            buffer.put(Handshake.BITTORRENT_PROTOCOL_IDENTIFIER.getBytes(Constants.BYTE_ENCODING));
            buffer.put(reserved);
            buffer.put(infoHash);
            buffer.put(peerId);
    ...
    public static Handshake parse(
            byte[] torrentInfoHash, byte[] clientPeerId, String torrentIdentifier)
            throws UnsupportedEncodingException, ParseException {
