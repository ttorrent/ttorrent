 ...
 * <p>
 *
 * <p>A BitTorrent client must check-in to the torrent's tracker(s) to get peers and to report
 * certain events.
 *
 * <p>
 *
 * <p>This Announce class implements a periodic announce request thread that will notify announce
...
public class Announce implements Runnable {
    ...
    public void forceAnnounce(
            AnnounceableInformation torrent,
            AnnounceResponseListener listener,
            AnnounceRequestMessage.RequestEvent event)
            throws UnknownServiceException, UnknownHostException {
        ...
        try {
            ...
            logger.info(
                    String.format(
                            "Unable to force announce torrent %s on tracker %s.",
                            torrent.getHexInfoHash(), String.valueOf(trackerUrl)));
            logger.debug(
                    String.format(
                            "Unable to force announce torrent %s on tracker %s.",
                            torrent.getHexInfoHash(), String.valueOf(trackerUrl)),
    ...
    public void start(
            ...
            final AnnounceResponseListener listener,
            final Peer[] peers,
        ...
        if (defaultTrackerURI != null) {
            try {
                myDefaultTracker =
                        myTrackerClientFactory.createTrackerClient(myPeers, defaultTrackerURI);
     ...
     * <p>
     *
     * <p>One last 'stopped' announce event might be sent to the tracker to announce we're going
     * away, depending on the implementation.
     */
    public void stop() {
     ...
     * <p>
     *
     * <p>The announce thread starts by making the initial 'started' announce request to register on
     * the tracker and get the announce interval value. Subsequent announce requests are ordinary,
     * event-less, periodic requests for peers.
     *
     * <p>
     *
     * <p>Unless forcefully stopped, the announce thread will terminate by sending a 'stopped'
     * announce request before stopping.
     */
    @Override
    public void run() {
        ...
        while (!this.stop && !Thread.currentThread().isInterrupted()) {
            ...
            final List<AnnounceableInformation> announceableInformationList =
                    myContext.getTorrentsStorage().announceableTorrents();
            logger.debug("Starting announce for {} torrents", announceableInformationList.size());
            announceAllTorrents(
                    announceableInformationList, AnnounceRequestMessage.RequestEvent.NONE);
            try {
        ...
        announceAllTorrents(
                myContext.getTorrentsStorage().announceableTorrents(),
                AnnounceRequestMessage.RequestEvent.STOPPED);
        ...
        for (AnnounceableInformation torrent : torrentsForAnnounce) {
            ...
            try {
                ...
                if (trackerClient != null) {
                    trackerClient.announceAllInterfaces(
                            AnnounceRequestMessage.RequestEvent.NONE, false, torrent);
                } else {
                    logger.warn(
                            "Tracker client for {} is null. Torrent is not announced on tracker",
    ...
    private void announceAllTorrents(
            List<AnnounceableInformation> announceableInformationList,
            AnnounceRequestMessage.RequestEvent event) {

        logger.debug(
                "Started multi announce. Event {}, torrents {}",
                event,
                announceableInformationList);
        final Map<String, List<AnnounceableInformation>> torrentsGroupingByAnnounceUrl =
                new HashMap<String, List<AnnounceableInformation>>();
        ...
        for (AnnounceableInformation torrent : announceableInformationList) {
            ...
            List<AnnounceableInformation> sharedTorrents =
                    torrentsGroupingByAnnounceUrl.get(torrentURI);
        ...
        List<AnnounceableInformation> unannouncedTorrents =
                new ArrayList<AnnounceableInformation>();
        for (Map.Entry<String, List<AnnounceableInformation>> e :
                torrentsGroupingByAnnounceUrl.entrySet()) {
            ...
            if (trackerClient != null) {
                try {
                    ...
                    LoggerUtils.warnAndDebugDetails(
                            logger, "problem in multi announce {}", t.getMessage(), t);
                    unannouncedTorrents.addAll(e.getValue());
                } catch (ConnectException t) {
                    LoggerUtils.warnWithMessageAndDebugDetails(
                            logger, "Cannot connect to the tracker {}", e.getKey(), t);
                    logger.debug(
                            "next torrents contain {} in tracker list. {}",
                            e.getKey(),
                            e.getValue());
                }
            } else {
                logger.warn(
                        "Tracker client for {} is null. Torrents are not announced on tracker",
                        e.getKey());
                if (e.getKey() == null || e.getKey().isEmpty()) {
                    for (AnnounceableInformation announceableInformation : e.getValue()) {
                        myContext
                                .getTorrentsStorage()
                                .remove(announceableInformation.getHexInfoHash());
                    }
                }
            }
        }
     ...
     * Stop the announce thread.
     *
     * @param hard Whether to force stop the announce thread or not, i.e. not send the final
     *     'stopped' announce request or not.
     */
