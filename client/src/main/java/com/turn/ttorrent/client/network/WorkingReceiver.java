...
public class WorkingReceiver implements DataProcessor {
    ...
    // So piece and bitfield have dynamic size because bytebuffer for this messages will be
    // allocated after get message length
    ...
    @NotNull private ByteBuffer messageBytes;
    ...
    public DataProcessor processAndGetNext(ByteChannel socketChannel) throws IOException {
        ...
        if (pstrLength == -1) {
            ...
            try {
                ...
                // Some clients close connection so that java throws IOException "An existing
                // connection was forcibly closed by the remote host"
                logger.debug("unable to read data from channel " + socketChannel, e);
                return new ShutdownAndRemovePeerProcessor(myPeerUID, myContext)
            ...
            if (this.pstrLength > MAX_MESSAGE_SIZE) {
                logger.warn(
                        ...
                        PeerMessage.MESSAGE_LENGTH_FIELD_SIZE + this.pstrLength,
                        MAX_MESSAGE_SIZE);
        ...
        if (PeerMessage.MESSAGE_LENGTH_FIELD_SIZE + this.pstrLength > messageBytes.capacity()) {
            ...
            old.rewind();
            messageBytes =
                    ByteBuffer.allocate(PeerMessage.MESSAGE_LENGTH_FIELD_SIZE + this.pstrLength);
        ...
        if (torrent == null || !myContext.getTorrentsStorage().hasTorrent(hexInfoHash)) {
            logger.debug(
                    "torrent with hash {} for peer {} doesn't found in storage. Maybe somebody"
                        + " deletes it manually",
                    hexInfoHash,
                    peer);
            return new ShutdownAndRemovePeerProcessor(myPeerUID, myContext)
        ...
        ByteBuffer bufferCopy =
                ByteBuffer.wrap(Arrays.copyOf(messageBytes.array(), messageBytes.limit()));
        ...
        try {
            ...
            LoggerUtils.warnAndDebugDetails(
                    logger, "incorrect message was received from peer {}", peer, e);
            return new ShutdownAndRemovePeerProcessor(myPeerUID, myContext)
                    .processAndGetNext(socketChannel);
        }
        ...
        try {
                                ...
                                public void run() {
                                    ...
                                    try {
                                        currentThread.setName(
                                                oldName
                                                        + " handle message for torrent "
                                                        + myPeerUID.getTorrentHash()
                                                        + " peer: "
                                        ...
                                        LoggerUtils.warnAndDebugDetails(
                                                logger,
                                                "unhandled exception {} in executor task"
                                                    + " (handleMessage)",
                                                e.toString(),
                                                e);
                                    } finally {
            ...
            LoggerUtils.warnAndDebugDetails(
                    logger, "task submit is failed. Reason: {}", e.getMessage(), e);
            return new ShutdownAndRemovePeerProcessor(myPeerUID, myContext)
                    .processAndGetNext(socketChannel);
        }
        return this;
