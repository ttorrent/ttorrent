...
public class EndGameStrategyImpl implements EndGameStrategy {
    ...
    public RequestsCollection collectRequests(Piece[] allPieces, List<SharingPeer> connectedPeers) {
        ...
        Collections.sort(
                sorted,
                new Comparator<SharingPeer>() {
                    @Override
                    public int compare(SharingPeer o1, SharingPeer o2) {
                        return Integer.valueOf(o1.getDownloadedPiecesCount())
                                .compareTo(o2.getDownloadedPiecesCount());
                    }
                });
        for (Piece piece : allPieces) {
            ...
            // (peers are selected by peer rank, peer with better rank will be selected more often
            // then peer with bad rank
    ...
    private List<SharingPeer> selectGoodPeers(
            Piece piece, int count, List<SharingPeer> sortedPeers) {
