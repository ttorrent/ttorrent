...
public abstract class TrackerClient {
    ...
    public void announceAllInterfaces(
        ...
        try {
            ...
            throw new AnnounceException(
                    String.format(
                            "Unable to announce tracker %s event %s for torrent %s and peers %s."
                                + " Reason %s",
                            getTrackerURI(),
                            event.getEventName(),
                            torrent.getHexInfoHash(),
                            Arrays.toString(myAddress.toArray()),
                            e.getMessage()),
                    e);
        }
    }
    ...
    protected abstract void announce(
            ...
            boolean inhibitEvent,
            final AnnounceableInformation torrent,
            final List<Peer> peer)
            throws AnnounceException;

    protected abstract void multiAnnounce(
            final AnnounceRequestMessage.RequestEvent event,
            boolean inhibitEvent,
    ...
    protected void logAnnounceRequest(
            AnnounceRequestMessage.RequestEvent event, AnnounceableInformation torrent) {
        if (event != AnnounceRequestMessage.RequestEvent.NONE) {
            ...
            logger.debug(
                    "Simply announcing to tracker with {}U/{}D/{}L bytes...",
                    new Object[] {
     ...
     *
     * <p>This method is called to make sure all connections are correctly closed when the announce
     ...
     *
     * <p>Analyzes the response from the tracker and acts on it. If the response is an error, it is
     ...
     */
    protected void fireAnnounceResponseEvent(
            int complete, int incomplete, int interval, String hexInfoHash) {
