 ...
 * <p>
 *
 * <p>The {@link SharedTorrent} class extends the Torrent class with all the data and logic required
 * by the BitTorrent client implementation.
 *
 * <p>
 *
 * <p><em>Note:</em> this implementation currently only supports single-file torrents.
 *
 * @author mpetazzoni
 */
public class SharedTorrent implements PeerActivityListener, TorrentMetadata, TorrentInfo {
    ...
    private static final RequestStrategy DEFAULT_REQUEST_STRATEGY =
            new RequestStrategyImplAnyInteresting();

    /**
     * End-game trigger ratio.
     *
     * <p>Eng-game behavior (requesting already requested pieces from available and ready peers to
     * try to speed-up the end of the transfer) will only be enabled when the ratio of completed
     * pieces over total pieces in the torrent is over this value.
     */
    ...
    public SharedTorrent(
        ...
        if (this.piecesHashes.capacity() / Constants.PIECE_HASH_SIZE * (long) this.pieceLength
                < myTorrentTotalSize) {
    ...
    public static SharedTorrent fromFile(
            File source, PieceStorage pieceStorage, TorrentStatistic torrentStatistic)
            throws IOException {
        byte[] data = FileUtils.readFileToByteArray(source);
        TorrentMetadata torrentMetadata = new TorrentParser().parse(data);
        return new SharedTorrent(
                torrentMetadata,
                pieceStorage,
    ...
    private synchronized void closeFileChannelIfNecessary() throws IOException {
        if (isFileChannelOpen && myDownloaders.size() == 0) {
            logger.debug(
                    "Closing file  channel for {} if necessary. Downloaders: {}",
                    getHexInfoHash(),
                    myDownloaders.size());
     ...
     * <p>
     *
     * <p><b>Note:</b> this could be more than the torrent's length, and should not be used to
     * determine a completion percentage.
     */
     ...
     * <p>
     *
     * <p>Hash and verify any potentially present local data and create this torrent's pieces array
     * from their respective hash provided in the torrent meta-info.
     *
     * <p>
     *
     * <p>This function should be called soon after the constructor to initialize the pieces array.
     */
    ...
    private void initPieces() {
        int nPieces = (int) (Math.ceil((double) myTorrentTotalSize / this.pieceLength));
    ...
    private void hashSingleThread() {
        ...
        logger.debug(
                "Analyzing local data for {} with {} threads...",
                myTorrentMetadata.getDirectoryName(),
        ...
        for (int idx = 0; idx < this.pieces.length; idx++) {
            ...
            long len = Math.min(myTorrentTotalSize - off, this.pieceLength);
     ...
     * <p>
     *
     * <p>Available pieces are pieces available in the swarm, and it does not include our own
     * pieces.
     */
     ...
     * <p>
     *
     * <p>This realizes the final, pre-seeding phase actions on this torrent, which usually consists
     * in putting the torrent data in their final form and at their target location.
     *
    ...
    public boolean isFinished() {
        return pieceStorage.getAvailablePieces().cardinality()
                == myTorrentMetadata.getPiecesCount();
    }
    ...
    /**
     * Mark a piece as completed, decrementing the piece size in bytes from our left bytes to
     * download counter.
     */
     ...
     * <p>
     *
     * <p>When a peer chokes, the requests made to it are canceled and we need to mark the
     * eventually piece we requested from it as available again for download tentative from another
     * peer.
     *
    ...
    public synchronized void handlePeerChoked(SharingPeer peer) {
        ...
        logger.trace(
                "Peer {} choked, we now have {} outstanding " + "request(s): {}.",
     ...
     * <p>
     *
     * <p>When a peer becomes ready to accept piece block requests, select a piece to download and
     * go for it.
     *
    ...
    private synchronized RequestsCollection getRequestsCollection(final SharingPeer peer) {
        if (myValidationFutures.size() > MAX_VALIDATION_TASK_COUNT)
            return RequestsCollection.Empty.INSTANCE;

        if (this.requestedPieces.cardinality() > MAX_REQUESTED_PIECES_PER_TORRENT)
            return RequestsCollection.Empty.INSTANCE;

        int completedAndValidated = pieceStorage.getAvailablePieces().cardinality();

        boolean turnOnEndGame =
                completedAndValidated > getPiecesCount() * ENG_GAME_COMPLETION_RATIO
        ...
        if (turnOnEndGame) {
            ...
            if (now - END_GAME_INVOCATION_PERIOD_MS > endGameEnabledOn) {
                ...
                logger.info(
                        "Running end-game mode, currently available {}/{} pieces",
                        pieceStorage.getAvailablePieces().cardinality(),
                        getPieceCount());
     ...
     * <p>
     *
     * <p>Handle updates in piece availability from a peer's HAVE message. When this happens, we
     * need to mark that piece as available from the peer.
     *
    ...
    public void handlePieceAvailability(SharingPeer peer, Piece piece) {
        boolean isPeerInteresting =
                !this.completedPieces.get(piece.getIndex())
     ...
     * <p>
     *
     * <p>Handle updates in piece availability from a peer's BITFIELD message. When this happens, we
     * need to mark in all the pieces the peer has that they can be reached through this peer, thus
    ...
    public void handleBitfieldAvailability(SharingPeer peer, BitSet availablePieces) {
        ...
        logger.debug(
                "Peer {} contributes {} piece(s), total pieces count: {}.",
                new Object[] {
                    peer, availablePieces.cardinality(), myTorrentMetadata.getPiecesCount()
                });
    }
     ...
     * <p>
     *
     * <p>When a piece has been sent to a peer, we just record that we sent that many bytes. If the
     * piece is valid on the peer's side, it will send us a HAVE message and we'll record that the
     * piece is available on the peer at that moment (see <code>handlePieceAvailability()</code>).
     *
     ...
     * <p>
     *
     * <p>If the complete piece downloaded is valid, we can record in the torrent completedPieces
     * bit field that we know have this piece.
     *
     * @param peer The peer we got this piece from.
     * @param piece The piece in question.
     */
    ...
    public void handlePieceCompleted(SharingPeer peer, Piece piece) throws IOException {
        ...
        logger.trace(
                "We now have {} piece(s) and {} outstanding request(s): {}",
                new Object[] {
                    this.completedPieces.cardinality(),
     ...
     * <p>
     *
     * <p>When a peer disconnects, we need to mark in all of the pieces it had available that they
     * can't be reached through this peer anymore.
     *
     * @param peer The peer we got this piece from.
     */
    @Override
    ...
    public synchronized void markCompletedAndAddValidationFuture(
            Piece piece, Future<?> validationFuture) {
