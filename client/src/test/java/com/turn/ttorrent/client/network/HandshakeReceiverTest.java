...
public class HandshakeReceiverTest {
    ...
    public HandshakeReceiverTest() {
        if (Logger.getRootLogger().getAllAppenders().hasMoreElements()) return;
        BasicConfigurator.configure(
                new ConsoleAppender(
                        new PatternLayout("[%d{MMdd HH:mm:ss,SSS} %t] %6p - %20.20c - %m %n")));
    }
    ...
    public void setUp() throws Exception {
        ...
        myHandshakeReceiver = new HandshakeReceiver(myContext, "127.0.0.1", 45664, false);
    }
    ...
    public void testReceiveHandshake() throws Exception {
        ...
        final SharedTorrent sharedTorrent =
                SharedTorrent.fromFile(
                        ...
                        FairPieceStorageFactory.INSTANCE.createStorage(
                                torrent,
                                FileCollectionStorage.create(torrent, tempFile.getParentFile())),
        ...
        when(myContext.createSharingPeer(
                        any(String.class),
                        anyInt(),
