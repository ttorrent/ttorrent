 ...
 * B-encoding decoder.
 *
 * <p>A b-encoded byte stream can represent byte arrays, numbers, lists and maps (dictionaries).
 * This class implements a decoder of such streams into {@link BEValue}s.
 *
 * <p>Inspired by Snark's implementation.
 *
 * @author mpetazzoni
...
public class BDecoder {
     ...
     *
     * <p>Nothing is read from the given <code>InputStream</code> yet.
     *
     ...
     *
     * <p>Automatically instantiates a new BDecoder for the provided input stream and decodes its
     * root member.
     *
     * @param in The input stream to read from.
     */
     ...
     *
     * <p>Automatically instantiates a new BDecoder for the provided buffer and decodes its root
     * member.
     *
     ...
     * been reached.
     *
     * <p>Can return something unexpected (not '0' .. '9', 'i', 'l' or 'd') when the stream isn't
     * b-encoded. This might or might not read one extra byte from the stream.
     */
    ...
    public BEValue bdecode() throws IOException {
        if (this.getNextIndicator() == -1) return null;

        if (this.indicator >= '0' && this.indicator <= '9') return this.bdecodeBytes();
        else if (this.indicator == 'i') return this.bdecodeNumber();
        else if (this.indicator == 'l') return this.bdecodeList();
        else if (this.indicator == 'd') return this.bdecodeMap();
        else throw new InvalidBEncodingException("Unknown indicator '" + this.indicator + "'");
    }
    ...
    public BEValue bdecodeNumber() throws IOException {
        ...
        if (c == '0') {
            c = this.read();
            if (c == 'e') return new BEValue(BigInteger.ZERO);
            else
     ...
     *     read. Note that the byte[] returned might be bigger then requested but will only contain
     *     length valid bytes. The returned byte[] will be reused when this method is called again.
     */
