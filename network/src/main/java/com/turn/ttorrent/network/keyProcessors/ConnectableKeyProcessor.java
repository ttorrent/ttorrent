...
public class ConnectableKeyProcessor implements KeyProcessor {
    ...
    private static final Logger logger =
            TorrentLoggerFactory.getLogger(ConnectableKeyProcessor.class);
    ...
    public ConnectableKeyProcessor(
            Selector selector,
            Clock timeService,
    ...
    public void process(SelectionKey key) throws IOException {
        ...
        if (!(attachment instanceof ConnectTask)) {
            logger.warn(
                    "incorrect instance of attachment for channel {}. The key for the channel is"
                        + " cancelled",
                    socketChannel);
            key.cancel();
            return;
        ...
        try {
            ...
            logger.info(
                    "Could not connect to {}:{}, received NoRouteToHostException",
                    connectTask.getHost(),
                    connectTask.getPort());
            ...
            logger.info(
                    "Could not connect to {}:{}, received ConnectException",
                    connectTask.getHost(),
                    connectTask.getPort());
            connectionListener.onError(socketChannel, e);
            return;
