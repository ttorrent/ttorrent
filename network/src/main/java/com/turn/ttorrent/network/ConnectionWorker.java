...
public class ConnectionWorker implements Runnable {
            ...
            while (!stop && (!Thread.currentThread().isInterrupted())) {
                try {
                    ...
                    LoggerUtils.warnAndDebugDetails(
                            logger,
                            "unable to select channel keys. Error message {}",
                            e.getMessage(),
                            e);
                }
            }
        ...
        while (iterator.hasNext()) {
            ...
            if (!socketChannel.isOpen()) {
                ...
                writeTask
                        .getListener()
                        .onWriteFailed(
                                getDefaultWriteErrorMessageWithSuffix(
                                        socketChannel, "Channel is not open"),
            ...
            if (key == null) {
                ...
                writeTask
                        ...
                        .onWriteFailed(
                                getDefaultWriteErrorMessageWithSuffix(
                                        socketChannel, "Can not find key for the channel"),
                                new ConnectionClosedException());
                continue;
            }
            ...
            if (!(attachment instanceof WriteAttachment)) {
                ...
                writeTask
                        ...
                        .onWriteFailed(
                                getDefaultWriteErrorMessageWithSuffix(
                                        socketChannel, "Incorrect attachment instance for the key"),
                                new ConnectionClosedException());
                continue;
            }
            ...
            if (keyAttachment.getWriteTasks().offer(writeTask)) {
                ...
                try {
                    ...
                    writeTask
                            ...
                            .onWriteFailed(
                                    getDefaultWriteErrorMessageWithSuffix(
                                            socketChannel, "Key is cancelled"),
                                    new ConnectionClosedException(e));
                }
            }
        }
    }

    private String getDefaultWriteErrorMessageWithSuffix(
            SocketChannel socketChannel, String suffix) {
        ...
        while ((connectTask = myConnectQueue.poll()) != null) {
            ...
            try {
                ...
                socketChannel.connect(
                        new InetSocketAddress(connectTask.getHost(), connectTask.getPort()));
            } catch (IOException e) {
                LoggerUtils.warnAndDebugDetails(
                        logger, "unable connect. Connect task is {}", connectTask, e);
            }
        }
    }
    ...
    private void processSelectedKeys() {
        ...
        for (SelectionKey key : selectionKeys) {
            ...
            try {
                ...
                logger.warn(
                        "error {} in processing key. Close channel {}",
                        e.getMessage(),
                        key.channel());
                logger.debug("", e);
                try {
    ...
    public boolean offerConnect(ConnectTask connectTask, int timeout, TimeUnit timeUnit) {
        if (!myNewConnectionAllower.isNewConnectionAllowed()) {
            logger.info(
                    "can not add connect task {} to queue. New connection is not allowed",
                    connectTask);
            return false;
        }
        ...
        if (!done) {
            writeTask
                    ...
                    .onWriteFailed(
                            "unable add task "
                                    + writeTask
                                    + " to the queue. Maybe queue is overload",
                            null);
        }
        return done;
    }

    private <T> boolean addTaskToQueue(
            T task, int timeout, TimeUnit timeUnit, BlockingQueue<T> queue) {
        try {
